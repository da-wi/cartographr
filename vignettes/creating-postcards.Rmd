...
df.coords <- df.coords |> 
  ...
  # 2. preprocess data
  mutate(osm_pps = list(preprocess_map(osm, circle = F))) |>
...
df.coords <- df.coords |> 
  ...
  mutate(plot = list(
    plot_map(osm_pps, get_color(color), scaling = get_scaling("A6"))+
    labs(title=name)+
...
ggarrange(plotlist = df.coords$plot, ncol = 3)
```
