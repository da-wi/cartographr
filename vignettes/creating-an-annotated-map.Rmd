...
p <- osm |> 
  ...
  geom_point(data = suppressMessages(sf::st_intersection(df_crime, soho_boundary)),
             aes(x = longitude, y = latitude, shape = type), color="#222424", size=2) + 
  theme_infomap_barlow() + 
  # Set labels
  labs(title = "CRIME IN SOHO (2023)",
       shape = "TYPE")
```

Now we can plot the map by simply calling its print method implicity:

```{r plot the map, fig.dim=c(8.3,11.7), fig.dpi = 72, out.width="100%"}
p
```
