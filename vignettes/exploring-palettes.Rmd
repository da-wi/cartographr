...
```{r load all palettes, eval = F, include = FALSE}
df_pal |> ungroup() |> mutate(i = row_number(), filename = sprintf("png/plot palettes-%d.png",i)) |> rowwise() |> mutate(res = ggsave(p + theme(plot.title = element_text(size = 75)), filename = sprintf("png/plot palettes-%d.png",i), device = "png",width = 160,height = 160, units = "mm", dpi = 150))
```
