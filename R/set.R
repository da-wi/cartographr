...
set_output_size <- function(size = NULL, orientation = "portrait") {
  ...
  if (is.character(size)) {
    if (!size %in% names(dims)) {
      cli::cli_abort('Format not recognized. Try: "A0", "A1", "A2", "A3", "A4", "A5", "A6", "small_poster", "medium_poster", "large_poster"')
    }
    else {
  ...
  if (is.numeric(size) && !grid::is.unit(size)) {
    if (length(size) != 2) {
      cli::cli_abort("Lenght of vector 'format' must be exactly 2.")
    }
    else {
...
set_attribution <- function(attribution = NULL) {
  ...
  if (!is.logical(attribution)) {
    cli::cli_abort("Argument must be type 'logical'")
  }
